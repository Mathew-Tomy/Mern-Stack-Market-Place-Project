    ...
    try {
        ...
        if (search !== '' && search !== undefined) {
            ...
            if (isSingleWord) {
                ...
                products = await Product.find({ active: true, title: { $regex: new RegExp('^' + search, 'i') } });
            } else {
                // Perform text search functionality for multi-word search terms
                products = await Product.find({ active: true, $text: { $search: search } });
            }
        } else {
            products = await Product.paginate({ active: true }, { page: parseInt(page) || 1, limit: 5 });
        }
        res.status(200).json({ products: products.docs || products, pages: products.pages });
    } catch (error) {
        console.error(error); // Log detailed error for debugging
        res.status(500).json({ message: "An error occurred. Please try again laters." });
    }
});
