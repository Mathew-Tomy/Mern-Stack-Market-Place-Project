...
export async function AdminUser(userData) {
...
export async function registerAdmin(userData) {
    return (await fetch(`${baseUrl}/auth/register-admin`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
...
export async function getAdmin() {
    return await (await fetch(baseUrl + '/auth/getAdmin', {credentials: 'include'})).json()
}
